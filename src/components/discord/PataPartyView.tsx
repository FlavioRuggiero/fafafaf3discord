...
const Dice = ({ value, rolling, diceType, size = 'md', players }: { value: number | string | number[], rolling: boolean, diceType?: string, size?: 'md' | 'sm', players?: Player[] }) => {
  ...
  return (
    <div className={`flex gap-1.5 ${rolling ? 'animate-dice-spin' : 'animate-dice-pop'}`}>
      {vals.map((v, idx) => {
        ...
        return (
          <div key={idx} className={containerClasses}>
            {isInteger1to6 ? (
              ...
              <span className={`${isSm ? 'text-xl' : 'text-4xl'} font-black text-[#111214] text-center`}>{v}</span>
            )}
          </div>
        );
      })}
                ...
                <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-[150] flex flex-col items-center gap-2 animate-in slide-in-from-bottom-5">
                  ...
                  <div className="flex gap-2 bg-black/20 p-2 rounded-2xl backdrop-blur-sm border border-white/10 shadow-xl">
                    ...
                    {players.find(p => p.id === user?.id)?.specialDice?.map((d, i) => {
                      ...
                      const label = d === 'ebete' ? 'Dado Ebete' : d === 'vigilante' ? 'Dado Vigilante' : d === 'frazionario' ? 'Dado Frazionario' : d === 'carismatico' ? 'Dado Carismatico' : d === 'doppio' ? 'Dado Doppio' : 'Dado di Scambio';
                      const desc = d === 'ebete' ? 'Può uscire 1, 1 o 6' : d === 'vigilante' ? 'Può uscire 3, 4 o 5' : d === 'frazionario' ? 'Può uscire 1.5, 2.5 o 3.5' : d === 'carismatico' ? 'Può uscire 😂😅😎😑🤑🥵😱' : d === 'doppio' ? 'Tira due dadi da 1 a 4' : 'Può uscire un giocatore a caso';
                      const shortLabel = d === 'scambio' ? 'S' : d === 'doppio' ? 'D' : d.charAt(0).toUpperCase();
