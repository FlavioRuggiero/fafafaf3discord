...
export const Avatar = ({ src, alt, className = "", decoration, isSpeaking, clipEffects }: AvatarProps) => {
  ...
  // Calcola gli stili di trasformazione (translate, rotate, scale) senza usare un wrapper
  // Questo permette di mantenere intatto il vero 3D (z-index) delle animazioni
  const getEffectStyle = (effect: BaseEffectConfig, baseStyle: React.CSSProperties = {}): React.CSSProperties => {
    ...
    // Se c'è un'icona personalizzata (emoji), rimuoviamo gli sfondi/bordi di default delle particelle
