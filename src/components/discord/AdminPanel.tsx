...
import { X, Search, Shield, Plus, Minus, Palette, Settings2, TrendingUp, PackageOpen, Ghost, Trash2 } from "lucide-react";
        ...
        <div className="flex-1 overflow-hidden flex flex-col">
          <div className="flex-shrink-0 flex border-b border-[#1f2023] bg-[#2b2d31] overflow-x-auto custom-scrollbar">
            ...
            <button 
              ...
              className={`px-6 py-3 text-sm font-bold transition-colors border-b-2 whitespace-nowrap ${activeTab === 'custom-editor' ? 'text-white border-brand' : 'text-[#b5bac1] border-transparent hover:text-[#dbdee1]'}`}
            >
              Contorni Custom
            </button>
          </div>
                ...
                <div className="bg-[#2b2d31] p-6 rounded-lg border border-[#1e1f22]">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 max-h-[60vh] overflow-y-auto custom-scrollbar pr-2">
                    {customDecorations.map(dec => (
                      <div key={dec.id} className="bg-[#1e1f22] p-4 rounded-lg border border-[#3f4147] flex flex-col justify-between">
                        <div className="flex items-center gap-3 mb-4">
                          <div className="w-12 h-12 rounded-full bg-[#2b2d31] flex items-center justify-center flex-shrink-0 border border-[#3f4147] overflow-hidden">
                            {dec.image_url ? (
                              <img src={dec.image_url} alt={dec.name} className="w-full h-full object-cover" />
                            ) : (
                              <Palette size={20} className="text-[#949ba4]" />
                            )}
                          </div>
                          <div className="min-w-0">
                            <h4 className="text-white font-bold truncate">{dec.name}</h4>
                            <div className="text-xs text-[#949ba4] flex items-center gap-1 mt-0.5">
                              <img src="/digitalcardus.png" alt="dc" className="w-3 h-3" />
                              {dec.price} DC
                            </div>
                          </div>
                        </div>
                        <button 
                          ...
                          className="w-full py-2 bg-[#f23f43]/10 hover:bg-[#f23f43] text-[#f23f43] hover:text-white rounded text-sm font-medium transition-colors flex items-center justify-center gap-2"
                        >
                    ...
                    {customDecorations.length === 0 && (
                      <div className="col-span-full text-center text-[#949ba4] py-8">
                        Nessun contorno custom presente.
                      </div>
                    )}
                  </div>
                </div>
