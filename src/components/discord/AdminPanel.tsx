...
export const AdminPanel = ({ onClose }: AdminPanelProps) => {
  ...
  useEffect(() => {
    if (currentUser) {
      const draft = localStorage.getItem(`admin_custom_dec_draft_${currentUser.id}`);
      if (draft) {
        try {
          const parsed = JSON.parse(draft);
          if (parsed.newDecName !== undefined) setNewDecName(parsed.newDecName);
          if (parsed.newDecPrice !== undefined) setNewDecPrice(parsed.newDecPrice);
  ...
  useEffect(() => {
    if (isDraftLoaded && currentUser) {
      const draft = {
        newDecName, newDecPrice, newDecBorder, newDecShadow, textColorType, newDecTextColor,
        newDecGradStart, newDecGradEnd, newDecAnim, baseEffects, elements, customAnimations
      };
      localStorage.setItem(`admin_custom_dec_draft_${currentUser.id}`, JSON.stringify(draft));
    }
  }, [newDecName, newDecPrice, newDecBorder, newDecShadow, textColorType, newDecTextColor, newDecGradStart, newDecGradEnd, newDecAnim, baseEffects, elements, customAnimations, isDraftLoaded, currentUser]);

  const handleClearDraft = () => {
    if (window.confirm("Sei sicuro di voler svuotare la bozza? Perderai tutte le modifiche non salvate.")) {
      localStorage.removeItem(`admin_custom_dec_draft_${currentUser?.id}`);
      setNewDecName('');
      setNewDecPrice(100);
